...
# Рекомендовал бы оптимизировать импорт, импортируя datetime из datetime
...
class CashCalculator(Calculator):
    ...
    def get_today_cash_remained(self, currency,
        ...
        # Если мы уверены, что на вход придет 100% один из трех заранее обозначенных типов валюты, то просто делаем else currency_type = 'руб' вместо elif.
        # Если не уверены, то надо обрабатывать ситуацию получения неправильного кода валюты.
        # Либо ещё вариант - сделать рубли дефолтным вариантом и изначально присваивать currency_type = 'руб' (в самом начале функции), после чего делать проверку на доллар и евро, так мы избавимся от вложенности здесь.
        # Но Я бы все равно лучше воспользовался словарями
        ...
        if cash_remained > 0:
            return (
                ...
                # А ещё ее консистентно: используем format или f строку? В двух местах по-разному.
        ...
        # Можно просто делать return без использования elif
            ...
            return 'Денег нет, держись:' \
                   ...
                   # Не консистентно: используем format или f строку? В двух местах по-разному.
